...
class Brick:
    def __init__(self, x, y, hit_points=1, brick_images=None, powerup_images=None, powerup_chance=1):
        self.brick_images = brick_images
        self.powerup_images = powerup_images
        ...
        if self.is_powerup_brick:
            self.powerup_type = random.choice(['expand', 'shrink', 'speed'])
    ...
    def maybe_drop_powerup(self):
        ...
        if self.is_powerup_brick:
            return (self.powerup_type, self.powerup_images[self.powerup_type])
        
        # Otherwise, there's still a random chance for a powerup to drop
        if random.random() < self.powerup_chance:
            powerup_type = random.choice(['expand', 'shrink', 'speed'])
            return (powerup_type, self.powerup_images[powerup_type])
